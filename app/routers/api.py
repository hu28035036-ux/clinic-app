...
def _therapist_only_codes_set(db) -> set:
    ...
    v1.3.37+: 기록필요(requires_record) 항목은 기록 기반 집계이므로 제외."""
    return {t.code for t in _all_treatments(db)
            if t.role == "therapist" and t.code != C.ESWT_CODE
            and not getattr(t, "requires_record", False)}
...
def update_config(payload: dict, db: Session = Depends(get_db), _: bool = Depends(require_admin)):
    ...
    # 앱(홈페이지) 이름 정규화 — 빈값이면 기본값 복원, 40자 상한(헤더 레이아웃 보호).
    if "app_title" in payload:
        t = str(payload.get("app_title") or "").strip()
        cfg["app_title"] = t[:40] if t else "병원 예약 관리"
...
def _serialize_treatment(t: models.Treatment) -> dict:
    ...
    return {
        ...
        # 기록 필요 (v1.3.37+). 기존 DB 에는 0(False) 으로 채워져 안전.
        "requires_record": bool(getattr(t, "requires_record", False)),
    }
...
def _build_treatment_meta(db) -> dict:
    ...
    # v1.3.37+: 기록필요 항목은 기록 기반 집계 → 도수치료에서 제외
    ...
    # 기록 필요 맵 + 기록필요 활성 코드 목록 (집계/기록 탭에서 경로 분리에 사용)
    treatment_requires_record = {
        t.code: bool(getattr(t, "requires_record", False)) for t in treatments
    }
    record_treatments = [
        t.code for t in treatments if t.active and getattr(t, "requires_record", False)
    ]
    ...
    return {
        ...
        # 기록 필요 (v1.3.37+)
        "treatment_requires_record": treatment_requires_record,
        "record_treatments": record_treatments,
...
def create_treatment(p: schemas.TreatmentIn, db: Session = Depends(get_db),
    ...
    t = models.Treatment(
        ...
        requires_record=bool(p.requires_record),
    )
    db.add(t); db.flush()
...
def update_treatment(tid: str, p: schemas.TreatmentIn, db: Session = Depends(get_db),
    ...
    t.requires_record = bool(p.requires_record)
    db.flush()
...
def delete_employee_duty(did: str, db: Session = Depends(get_db)):
    obj = db.get(models.EmployeeDuty, did)
    if not obj:
        raise HTTPException(404, "당직 데이터가 없습니다.")
    db.delete(obj)
    _log(db, "employee_duty", did, "delete", None)
    db.commit()
    return {"ok": True}
...
@router.post("/employee-duties/bulk-add")
def bulk_add_employee_duties(payload: dict, db: Session = Depends(get_db)):
    """여러 (employee_id, duty_date) 당직을 한 번에 upsert.
    ...
    for item in items:
        ...
        if not emp_id or not duty_date:
            continue
...
def patients_count(db: Session = Depends(get_db)):
    """환자 수만 반환 — 환자 탭 (환자 수) 배지용 경량 카운트.

    대량(수만 건) 환경에서 배지 표시를 위해 전체 환자를 light 로드하던 것을
    피하기 위한 단순 COUNT. (라우트 순서상 ``/patients/{pid}`` 보다 먼저 정의.)
    """
    return {"count": db.query(models.Patient).count()}
...
def search_patients(q: str = "", field: str = "all", limit: int = 20, offset: int = 0,
    ...
    counts_show = _counts_show_map(db, [p.id for p in rows])
    items = [
        {"id": p.id, "name": p.name, "chart_no": p.chart_no,
         "phone": p.phone, "birth_date": p.birth_date,
         "gender": getattr(p, "gender", "") or "",
         ...
         "counts_show": counts_show.get(p.id, [])}
        for p in rows
    ]
    return {
...
def _counts_show_map(db, patient_ids) -> dict:
    """환자탭 목록 '완료 횟수' 표시용 맵 — {pid: [완료(done>0) 치료항목]}.

    환자 목록은 search/by-ids 로 페이지 단위(≤500명)만 받으므로 IN 쿼리로 가볍다
    (전체 PatientTreatmentCount 로드 회피). 표시 ON·활성·완료>0 항목만, code 순.
    (search_patients / patients_by_ids 공용 — 이게 없으면 완료횟수가 '-' 로만 보임.)
    """
    from collections import defaultdict
    ids = [pid for pid in (patient_ids or []) if pid]
    if not ids:
    ...
    treatments = (db.query(models.Treatment)
                  .filter(models.Treatment.show_in_patient == True,  # noqa: E712
                          models.Treatment.active == True)  # noqa: E712
                  .order_by(models.Treatment.sort_order).all())
    by_pid = defaultdict(dict)
    for i in range(0, len(ids), 500):  # SQLite IN ~999 한계 대비 청크
        chunk = ids[i:i + 500]
        for c in (db.query(models.PatientTreatmentCount)
                  .filter(models.PatientTreatmentCount.patient_id.in_(chunk)).all()):
            by_pid[c.patient_id][c.treatment_id] = c
    out: dict = {}
    for pid in ids:
        rc = by_pid.get(pid, {})
        ...
        for t in treatments:
            c = rc.get(t.id)
            done = c.done_count if c else 0
            if done <= 0:
                continue  # 완료 0 은 표시 안 함(_pmDoneCompact 와 동일) — 응답 경량화
            items.append({
                "treatment_id": t.id, "code": t.code, "name": t.name,
                "short": t.short, "role": t.role,
                "show": t.show_in_patient, "active": t.active,
                "rx_count": c.rx_count if c else 0, "done_count": done,
            })
...
def get_appointment(aid: str, db: Session = Depends(get_db)):
    """단건 예약 조회 (예약 상세 모달용).

    대량 데이터(연간 수만 건) 환경에서 상세를 열 때 연간 전체 목록을 받아
    JS 에서 find 하던 비효율을 PK 단건 조회로 대체 — 응답·전송량이 데이터 양과 무관.
    """
    a = db.get(models.Appointment, aid)
    if not a:
        raise HTTPException(404, "예약을 찾을 수 없습니다.")
    return _serialize_appointment(a)
...
_DC_HEADER_ALIASES = {
    ...
    # 차트번호 = 환자번호. 외부 EMR 내보내기는 '환자번호' 컬럼명을 쓰는 경우가 많아 함께 인식.
    "chart_no":   ["차트", "차트번호", "차번", "chart", "chart_no", "chart no", "차트 번호",
                   "환자번호", "환자 번호", "환자no", "환자 no", "patient_no", "patient no",
                   "patientno", "환자id", "patient_id"],
...
def _dc_birth_from_ssn(raw_ssn):
    """주민번호(13자리, '-' 유무 무관) 앞 6자리로 생년월일(YYYY-MM-DD) 추정.

    - 앞 6자리 = YYMMDD
    - 7번째 숫자(성별/세기 코드)로 세기 구분:
        1,2,5,6 → 1900년대 / 3,4,7,8 → 2000년대 / 9,0 → 1800년대
    - 7번째 숫자가 없으면(6자리만) 연도 30 기준으로 추정 (날짜 보조 판정과 동일)
    - 월/일 범위가 비정상이면 None (검토 필요로 분류됨)
    """
    if raw_ssn in (None, ""):
        return None
    import re as _re
...
def _dc_dupe_key_in_file(e):
    """파일 내부 중복 판정용 키 — 차트번호(=환자번호) 우선, 없으면 이름+생년월일."""
    if e.get("chart_no"):
...
def _dc_is_duplicate_in_db(e, chart_set, key_set):
    ...
    - 차트번호(=환자번호)가 기존 환자와 같으면 중복
    - 또는 이름 + 생년월일이 기존 환자와 같으면 중복
    """
...
def _dc_db_match(e, chart_map, key_map):
    """DB 기존 환자 대비 중복 판정 + 어떤 환자와 겹쳤는지 반환 (v1.3.51+).

    판정 규칙은 ``_dc_is_duplicate_in_db`` 와 동일 (차트번호 우선, 없으면
    이름+생년월일). 겹치면 {"matched_by", "db_name", "db_chart_no",
    "db_birth_date"} — 미리보기에서 '겹치는 환자 내역' 표시에 사용.
    겹치지 않으면 None.
    """
    if e.get("chart_no") and e["chart_no"] in chart_map:
        p = chart_map[e["chart_no"]]
        return {
            "matched_by": "차트번호",
            "db_name": p[0] or "",
            "db_birth_date": p[1] or "",
            "db_chart_no": p[2] or "",
        }
    key = (
        (e.get("name") or "").strip(),
...
def _dc_parse_excel(file_bytes: bytes):
    ...
    for r_idx, row in enumerate(rows[data_start:], start=data_start + 1):
        ...
        # 생년월일 컬럼이 없거나 비어 있으면 주민번호 앞 6자리로 보완
        if not birth:
            birth = _dc_birth_from_ssn(_get("ssn"))
...
async def data_convert_preview(
    ...
    # 기존 환자 색인 — 중복 판정에 필요한 3개 컬럼만 로드(대량 환경 메모리 절감).
    # v1.3.51+: set 대신 map — '어떤 기존 환자와 겹쳤는지' 내역 표시용.
    pats = db.query(models.Patient.name, models.Patient.birth_date,
                    models.Patient.chart_no).all()
    chart_map = {p.chart_no: (p.name, p.birth_date, p.chart_no)
                 for p in pats if p.chart_no}
    key_map = {
        ((p.name or "").strip(), (p.birth_date or "")): (p.name, p.birth_date, p.chart_no)
    ...
    new_list = []
    existing_list = []  # v1.3.51+: 기존 DB 환자와 겹친 엑셀 행 내역
    existing_count = 0
    dup_in_file_count = 0
    dup_in_file = set()
    for e in entries:
        ...
        if k in dup_in_file:
            ...
            continue
        dup_in_file.add(k)
        m = _dc_db_match(e, chart_map, key_map)
        if m is not None:
            existing_count += 1
            existing_list.append({**e, **m})
        else:
    ...
    return {
        ...
        "existing_patients": existing_list,
        "dup_in_file_count": dup_in_file_count,
...
def data_convert_apply(
    ...
    # 적용 직전 DB 색인 다시 구축 — 필요한 3개 컬럼만 로드(대량 환경 메모리 절감).
    pats = db.query(models.Patient.name, models.Patient.birth_date,
                    models.Patient.chart_no).all()
    chart_set = {p.chart_no for p in pats if p.chart_no}
    key_set = {
        ((p.name or "").strip(), (p.birth_date or ""))
        for p in pats if p.name
    }
...
def _get_manual_treatment_rows(db):
    ...
    return (db.query(models.Treatment)
            .filter(models.Treatment.role == "therapist",
                    models.Treatment.code != C.ESWT_CODE,
                    models.Treatment.requires_record == False,  # noqa: E712
                    models.Treatment.active == True)
            .order_by(models.Treatment.sort_order).all())
...
def stats_direct_aggregate(date_from: str = "", date_to: str = "",
                           category_id: str = "",
                           db: Session = Depends(get_db)):
    """집계 탭 자동계산 (v1.3.37+): 과별 치료항목 × 직원 × 날짜.

    - 모든 치료항목: 치료완료(approved) 예약 건수 (코드별 담당자 귀속).
    - 기록필요 치료항목: record_entries 건수를 위 예약 건수에 **합산**.
      (v1.3.51+: 기존 '경로 분리(둘 중 하나만)' → '치료완료+기록 합산' 으로 변경)
    - manual_counts: 위 자동값에 더하는 수동 보정(walk-in) 델타.
    셀별로 합계(counts) + 자동값(auto) + 수동값(manual) 을 함께 내려준다.
    """
    from collections import defaultdict
    ...
    record_codes = {t.code for t in treatments if getattr(t, "requires_record", False)}

    # auto[date][tid][code] = 자동 집계 (예약 완료 + 기록)
    auto = defaultdict(lambda: defaultdict(lambda: defaultdict(int)))

    # (A) 모든 항목 — 치료완료(approved) 예약 자동 집계.
    #     코드별 담당자 = assignment handler, 없으면 주담당 therapist_id.
    #     v1.3.51+: 기록필요 항목도 예약 완료 건수를 포함 (기록 건수와 합산).
    if valid_codes:
        appt_rows = (
        ...
        for a in appt_rows:
            dk = a.start_at.strftime("%Y-%m-%d")
            asn_map = {x.treatment_code: x.handler_id for x in a.assignments}
            for c in _parse_codes(a.treatment_codes):
                if c not in valid_codes:
                    continue
                handler = asn_map.get(c) or a.therapist_id
                tid = handler or "__none__"
                auto[dk][tid][c] += 1

    # (B) 기록필요 항목 — record_entries 직원·날짜별 건수를 (A) 에 합산 (tab_key = 치료항목 code).
    if record_codes:
        rec_rows = (
            db.query(models.RecordEntry)
            .filter(models.RecordEntry.record_date >= mc_start_str,
                    models.RecordEntry.record_date < mc_end_str)
            .all()
        )
        for r in rec_rows:
            if r.tab_key not in record_codes:
                continue
            tid = r.employee_id or "__none__"
            auto[r.record_date][tid][r.tab_key] += 1

    # (C) 수동 보정(manual_counts) — 자동값 위에 더하는 walk-in 델타.
    mc_rows = (
    ...
    manual = defaultdict(lambda: defaultdict(lambda: defaultdict(int)))
    for mc in mc_rows:
        if mc.treatment_code not in valid_codes:
            continue
        tid = mc.therapist_id or "__none__"
        manual[mc.count_date][tid][mc.treatment_code] = max(0, int(mc.count or 0))

    items = []
    for dk in date_keys:
        employee_data = {}
        for e in selected_employees:
            a_counts = {code: auto[dk][e.id].get(code, 0) for code in treatment_codes}
            m_counts = {code: manual[dk][e.id].get(code, 0) for code in treatment_codes}
            employee_data[e.id] = {
                # counts = 합계(자동+수동). 기존 프론트 호환.
                "counts": {code: a_counts[code] + m_counts[code] for code in treatment_codes},
                "auto": a_counts,      # 자동값 (치료완료/기록) — 읽기전용 표시
                "manual": m_counts,    # 수동 보정값 (walk-in)
            }
    ...
    return {
        ...
        "record_codes": sorted(record_codes),
        "items": items,
    }
