...
class EmployeeDuty(Base):
    ...
    duty_type 으로 아침당직(morning) / 점심당직(lunch) / 야간당직(night) 분리 —
    같은 직원이 같은 날 여러 유형을 설 수 있음. 정보성 — 예약 차단/통계 무관."""
...
class RevenueRecord(Base):
    ...
    receivable_income = Column(Integer, nullable=False, default=0)
    ...
    certificate_amount = Column(Integer, nullable=False, default=0)
    disability_fund = Column(Integer, nullable=False, default=0)
    uninsured_amount = Column(Integer, nullable=False, default=0)
    meal_amount = Column(Integer, nullable=False, default=0)
    other_amount = Column(Integer, nullable=False, default=0)
    discount_amount = Column(Integer, nullable=False, default=0)
    free_amount = Column(Integer, nullable=False, default=0)
    cash_expense_amount = Column(Integer, nullable=False, default=0)
    field_memos_json = Column(Text, nullable=False, default="{}")
    memo = Column(Text, nullable=False, default="")
    created_at = Column(DateTime, default=datetime.utcnow)
    updated_at = Column(DateTime, default=datetime.utcnow, onupdate=datetime.utcnow)
