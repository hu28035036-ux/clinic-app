...
## 10. 현재 릴리스 상태 (v1.3.30 · 2026-06-15)

- **미니달력 깨짐 수정**: 고정 높이로 인한 내용 넘침/잘림 → `height: auto` + 휴무명 3개 축약(+N, 툴팁 전체).
- **금일 목록/상세창 "?" 수정**: 신규 환자 등록 직후 캐시 미동기 시 환자명이 `?`로 나오던 문제 → 서버 임베드 `patient_name`/`chart_no` 폴백.
- **검증**: 전체 회귀 **2,241 passed**, ruff 통과, `node --check` 통과, 격리 exe 스모크(HTTP 200·v1.3.30) 통과.
